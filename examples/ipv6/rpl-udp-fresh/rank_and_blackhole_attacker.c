...
extern uint8_t dio_smaller_than_dag; // if version attack, this will be 
...
PROCESS_THREAD(udp_client_process, ev, data)
{
  ...
  PRINTF(" local/remote port %u/%u\n",
	...
	printf("MAL-NODE: DATA Intercept: %d, MALICIOUS_LEVEL:%d, GREY_SINK_HOLE_ATTACK: %d\n",
						intercept_on, MALICIOUS_LEVEL, GREY_SINK_HOLE_ATTACK);
	printf("if GREY_SINK_HOLE_ATTACK == 0 ==> BLACK_SINK_HOLE_ATTACK ON\n");
...
/***** Check the 2nd thread for manual start/stop of malicious activities */
		if (counter == 4){ //start malicious behavior
			 intercept_on = 1;
			...
			if(intercept_on == 1){
			  if(GREY_SINK_HOLE_ATTACK == 1){
					...
					if(randomSend < 50 ){ //it seems more trully random like this...
						ctimer_set(&backoff_timer, SEND_TIME, send_packet, NULL); 
#if PRINT_DETAILS
						printf("MAL-NODE: my UDP sent to sink...\n");  
#endif
					}else{ 
#if PRINT_DETAILS
						printf("MAL-NODE: my UDP NOT sent\n");
#endif
					}
			 		...
			 		printf("MAL-NODE: Blackhole attack ON, My msg dropped...\n");
						...
						ctimer_set(&backoff_timer, SEND_TIME, send_packet, NULL); 
#if PRINT_DETAILS
						printf("MAL-NODE in normal operation...\n");  
#endif
			}	
    }
...
{
  /* Turn on/off the malicious activity via the buttons */
  PROCESS_BEGIN();
  active = 0;
